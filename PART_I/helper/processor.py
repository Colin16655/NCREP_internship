...
class PeakPicker:
    ...
    def plot_debug(self, band, freqs, S, f_domain, MAC_modified, mode_ranges, selected_ranges, p, dt):
        ...
        # Apply step size to frequency labels
        selected_indices = freq_indices[::step_size]
        selected_freqs = freqs[f_domain][::step_size]
        freq_labels = [f'{f:.1f}' for f in selected_freqs]  # Round frequency labels

        cax = ax1.imshow(MAC_modified, interpolation='none', aspect='equal')
        fig1.colorbar(cax, ax=ax1, label='MAC Value')

        # Set ticks only at selected indices
